...
function renderShop(){
  ...
  var bLabel='<div style="font-size:12px;opacity:0.6;margin-bottom:5px;color:'+_cCols[bt]+'">'+t('shop_blocks_label')+' — '+blockCost+' '+_cName(bt)+'</div><div style="display:flex;gap:4px">';
  bLabel+='<button data-action="buy-coal" style="'+bs+(okB?'color:#c8c4e8"':'color:#444;cursor:not-allowed"')+'>'+_cName('coal')+'</button>';
  bLabel+='<button data-action="buy-gold" style="'+bs+(okB?'color:#f5c830"':'color:#444;cursor:not-allowed"')+'>'+_cName('gold')+'</button>';
  bLabel+='<button data-action="buy-diamond" style="'+bs+(okB?'color:#80eeff"':'color:#444;cursor:not-allowed"')+'>'+_cName('diamond')+'</button>';
  bLabel+='</div>';
...
function buyBlock(btype){
  ...
  if((p[bt]||0)<cost){log(t('log_cancel')+' '+_cName(bt)+' ('+cost+')');return;}
  ...
  _showPlaceInfo('BLOC '+{coal:_cName('coal'),gold:_cName('gold'),diamond:_cName('diamond')}[btype]);
}
